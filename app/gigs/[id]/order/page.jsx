...
function OrderContent() {
  ...
  const orderDetails = {
    ...
    packageDesc: searchParams.get("packageDesc"),
    ...
    // This expects the JSON string we sent from the source page
    features: JSON.parse(searchParams.get("packageFeatures") || "[]"),
    gigTitle: searchParams.get("gigTitle"),
    gigId: searchParams.get("gigId"),
    gigImage: searchParams.get("gigImage"),
  };
  const serviceFee = (orderDetails.price * 0.01).toFixed(2);
            ...
            <div className="bg-white p-8 rounded-lg border border-gray-200 shadow-sm mb-6">
              ...
              <div className="flex gap-6">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 mb-2 leading-snug">
                    {orderDetails.gigTitle}
                  </h3>
                  <div className="flex items-center gap-2 text-sm">
                    <span className="bg-[#1dbf73]/10 text-[#1dbf73] px-2 py-1 rounded font-semibold">
                      {orderDetails.packageName} Package
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 border-t border-gray-100 pt-6">
                <h4 className="font-bold text-gray-900 mb-4">
                  ...
                  <p className="flex justify-between text-gray-600 font-normal">
                    {orderDetails.packageDesc}
                  </p>
                </h4>
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {orderDetails.features.map((feature, i) => (
                    ...
                    >
                      <svg
                        className="w-4 h-4 text-[#1dbf73] flex-shrink-0"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                  ...
                  <li className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    {orderDetails.delivery} Delivery Time
                  </li>
                  <li className="flex items-center gap-2 text-sm text-gray-600">
                    <svg
                      className="w-4 h-4 text-[#1dbf73] flex-shrink-0"
                    ...
                    {orderDetails.revisions} Revisions
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-lg border border-gray-200 shadow-sm sticky top-32">
              ...
              <div className="border-t border-gray-100 pt-4 mb-8">
                ...
                <span className="text-xs text-gray-400 mt-1 block">
                  Delivery time: {orderDetails.delivery}
                </span>
              </div>
              ...
              <button
                onClick={() => {
                  ...
                  router.push(
                    `/gigs/${orderDetails.gigId}/payment?${params.toString()}`,
                  );
                }}
